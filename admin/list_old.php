...
/* 페이징 시작 */
    ...
    // 등교 가능여부
    if($_GET['gowork'] == 'Y') $gowork = "user_val = '0' AND";
    else if($_GET['gowork'] == 'N') $gowork = "user_val < 0 AND";
    else $gowork = "";

    $sql = "select * from check_report where $gowork regdate = '$days' order by nid desc"; 
    $result = $db->query($sql);
...
<?
    ...
    if($currentSection != 1) { 
        $paging .= '<a class="icon item"  href="./list.php?page=' . $prevPage . '"><i class="left chevron icon"></i></a>';
    }    
    for($i = $firstPage; $i <= $lastPage; $i++) {
        if($i == $page) {
            $paging .= '<a class="item">' . $i . '</a>';
        } else {
            $paging .= ' <a class="item" href="./list.php?page=' . $i . '">' . $i . '</a>';
        }
    }

    //마지막 섹션이 아니라면 다음 버튼을 생성
    if($currentSection != $allSection) { 
        $paging .= '<a class="icon item" href="./list.php?page=' . $nextPage . '"><i class="right chevron icon"></i></a>';
    }    
        ...
        <div id="container">
            ...
            <div class="main ui intro container">
                <div class="ui row" style="padding:10px 0px;">
                    <form class="ui form" action="/admin/list.php" method="GET">
